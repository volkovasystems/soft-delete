...
elif [[ -f "/usr/local/share/soft-delete/VERSION" ]]; then
    VERSION_FILE="/usr/local/share/soft-delete/VERSION"
elif [[ -f "/usr/share/soft-delete/VERSION" ]]; then
    VERSION_FILE="/usr/share/soft-delete/VERSION"
else
    VERSION_FILE=""
fi
readonly VERSION_FILE

# Read version from VERSION file or embedded version
if [[ -f "$VERSION_FILE" ]]; then
    VERSION=$(tr -d '\n\r' < "$VERSION_FILE" | tr -d ' ')
else
    # Embedded version for binary distribution
    VERSION="1.0.0"
fi
readonly VERSION
